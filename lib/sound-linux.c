...
void sound_set_volume(uint8_t channel, uint8_t volume) {
	if (channel >= SOUND_MAX_CHANNELS) return;
	...
	if (sound_buffer_volume_sum < 255) sound_buffer_volume_sum = 255;
	for (uint8_t i=0;i<SOUND_MAX_CHANNELS;i++) {
		_sound_buffer_volume[i] = 255 * sound_buffer_volume[i] / sound_buffer_volume_sum;
	}
}
...
void sound_set_hz(uint8_t channel, float hz) {
	...
	if (hz < 20) {
		...
		newspeed = finetune*256.0 * 256.0 * sound_buffer_len * hz / (SOUND_BITRATE * sound_buffer_hz);
		#if SOUND_ADRS_ENABLED == 1
...
uint16_t adsr_volume(uint8_t channel) {
	if (channel > SOUND_MAX_CHANNELS) return 0;
...
volatile uint8_t sound_process_one_sample(int8_t pan) {
	...
	for (uint8_t i=0;i<SOUND_MAX_CHANNELS;i++) {
		...
		uint8_t sample_cursor = (sound_buffercursor[i] >> 8) % SOUND_MAXIMUM_SAMPLE_LENGTH;

		int8_t sample = sound_buffer[waveform][sample_cursor];
		#if SOUND_ADRS_ENABLED == 1
		uint8_t volume = sound_channel_adsr_state[i] == ADSR_OFF ? _sound_buffer_volume[i] : adsr_volume(i);
		#else
    ...
    float pan_factor = (pan ? abs(-127 - sound_buffer_pan[i]) : abs(127 - sound_buffer_pan[i])) / 254.0;
		_bufsum += sample * volume * pan_factor;
		sound_buffercursor[i] = (sound_buffercursor[i] + sound_buffer_speed[i] * (1 + sound_truncate_count));
	}
	sound_truncate_count = 0;

  int8_t sample = _bufsum >> 6;
  int8_t delay_sample = (delay_buffer[pan][delay_buffer_cursor] / 1.3) +
  (delay_buffer[pan][(delay_buffer_cursor + DELAY_BUFFER_LENGTH + 1026) % DELAY_BUFFER_LENGTH] / 4 + 96) +
  (delay_buffer[pan][(delay_buffer_cursor + DELAY_BUFFER_LENGTH + 1226) % DELAY_BUFFER_LENGTH] / 8 + 112) +
  (delay_buffer[pan][(delay_buffer_cursor + DELAY_BUFFER_LENGTH + 626) % DELAY_BUFFER_LENGTH] / 4 + 96) +
  (delay_buffer[pan][(delay_buffer_cursor + DELAY_BUFFER_LENGTH + 777) % DELAY_BUFFER_LENGTH] / 8 + 112) +
  (delay_buffer[pan][(delay_buffer_cursor + DELAY_BUFFER_LENGTH + 274) % DELAY_BUFFER_LENGTH] / 8 + 112) +
  (delay_buffer[pan][(delay_buffer_cursor + DELAY_BUFFER_LENGTH + 626) % DELAY_BUFFER_LENGTH] / 4 + 96) +
  (delay_buffer[pan][(delay_buffer_cursor + DELAY_BUFFER_LENGTH + 1526) % DELAY_BUFFER_LENGTH] / 8 + 112);

  delay_buffer[pan][delay_buffer_cursor] = ((sample + (delay_sample * DELAY_FACTOR * 0.3)));
  delay_buffer_cursor++;
